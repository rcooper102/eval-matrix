...
export class ScoreQuestion extends React.Component {
  ...
  render() {
    ...
    let result = null;
    if(score) {
      result = <h6><strong>{ score !== -1 ? question[Math.round(score * (question.length - 1))] : 'No Answer' }</strong><span className='progress'><div className="bar" style={{ width: `${score*100}px`}}></div><div className='label'>{ score !== -1 ? `${Math.round(score*100)}%` : 'n/a' }</div></span></h6>
    }
